...
function ContactCard({ icon, title, detail, link }: ContactCardProps) {
  const content = (
    <div className="p-6 bg-gray-50/50 dark:!bg-[#12151B] border border-gray-100 dark:!border-[#B89968]/15 rounded-xl hover:bg-white dark:hover:!bg-[#181C25] hover:shadow-xl hover:shadow-[#5b1f1f]/5 dark:hover:!border-[#B89968]/40 transition-all duration-500 group">
...
export default function ContactUs() {
  ...
  return (
    <section
      ref={sectionRef}
      className="min-h-screen pt-32 pb-24 px-6 bg-white dark:bg-[#0B0E14] text-[#1a1a1a] dark:text-gray-200 font-serif transition-colors duration-300"
    >
      <SeoManager
        title={t.seoTitle}
      ...
      <div className={`max-w-[1200px] mx-auto transition-all duration-1000 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        <div className="text-center mb-20">
          ...
          <h2
            className="text-5xl md:text-6xl font-light text-[#1a1a1a] tracking-tighter"
            dangerouslySetInnerHTML={{ __html: t.mainTitle }}
          />
          <div className="w-12 h-[1px] bg-[#5b1f1f]/30 mx-auto mt-8" />
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          <div className="space-y-8">
            <p className="text-gray-500 text-lg leading-relaxed mb-10">{t.intro}</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <ContactCard icon={<Mail className="w-5 h-5" />} title={t.email} detail="rklamiani@gmail.com" link="mailto:rklamiani@gmail.com" />
              <ContactCard icon={<Phone className="w-5 h-5" />} title={t.call} detail="+995 599 47 20 67" link="tel:+995599472067" />
              <ContactCard icon={<MapPin className="w-5 h-5" />} title={t.visit} detail={t.address} />
              <ContactCard icon={<Clock className="w-5 h-5" />} title={t.hours} detail={t.time} />
            </div>
          </div>
          ...
          <div className="relative group rounded-2xl overflow-hidden shadow-2xl shadow-[#5b1f1f]/5 border border-gray-100">
            ...
            />
            <div className="absolute inset-0 pointer-events-none border-[15px] border-white/10 backdrop-blur-[1px]" />
          </div>
        </div>
      </div>
    </section>
