  ...
  return (
    <main className="max-w-[1300px] mx-auto px-4 sm:px-6 lg:px-6 py-16 sm:py-20 lg:py-24 bg-white dark:bg-[#12151B] text-[#1a1a1a] dark:text-gray-200 font-serif transition-colors duration-300">
      ...
      <div className="text-center mb-10 sm:mb-12 md:mb-16 space-y-3 sm:space-y-4">
        ...
        <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-light text-[#1a1a1a] tracking-tighter italic"
            dangerouslySetInnerHTML={{ __html: t.title }}>
        </h2>
        <div className="w-10 sm:w-12 h-[1px] bg-[#b38b59]/30 mx-auto mt-4 sm:mt-6"></div>
      </div>

      {/* MOBILE + TABLET SWIPER (responsive breakpoints) */}
            ...
            <SwiperSlide key={wine.id}>
              <div className="relative aspect-[3/4] rounded-sm overflow-hidden shadow-xl shadow-black/5 dark:!bg-[#12151B] dark:ring-1 dark:ring-[#B89968]/20 dark:shadow-[0_18px_50px_rgba(0,0,0,0.7),0_0_0_1px_rgba(184,153,104,0.08)_inset]">
                <img
                  ...
                  decoding="async"
                />
                {/* Vignette + bottom fade — dark only */}
                <div className="hidden dark:block absolute inset-0 pointer-events-none [background:radial-gradient(ellipse_at_center,transparent_55%,rgba(11,14,20,0.55)_100%)]"></div>
                <div className="hidden dark:block absolute inset-x-0 bottom-0 h-1/3 pointer-events-none bg-gradient-to-t from-[#0B0E14]/85 via-[#0B0E14]/30 to-transparent"></div>
                {/* Top edge brass shimmer */}
                <div className="hidden dark:block absolute top-0 inset-x-0 h-px pointer-events-none bg-gradient-to-r from-transparent via-[#B89968]/45 to-transparent"></div>
              </div>
            </SwiperSlide>
          ))}
        </Swiper>
      ...
      <div className="hidden md:grid grid-cols-3 gap-6 mb-32">
        {wineImages.map((wine, idx) => (
          <div
            key={wine.id}
            className={`relative group overflow-hidden rounded-sm bg-gray-50 dark:!bg-[#12151B] dark:ring-1 dark:ring-[#B89968]/20 shadow-sm transition-all duration-700 hover:shadow-2xl hover:shadow-[#5b1f1f]/10 ${
          ...
          >
            ...
            {/* Dark vignette frame */}
            <div className="hidden dark:block absolute inset-0 pointer-events-none [background:radial-gradient(ellipse_at_center,transparent_50%,rgba(11,14,20,0.6)_100%)]"></div>
            {/* Dark bottom fade */}
            <div className="hidden dark:block absolute inset-x-0 bottom-0 h-2/5 pointer-events-none bg-gradient-to-t from-[#0B0E14]/90 via-[#0B0E14]/40 to-transparent"></div>
            {/* Dark top brass shimmer */}
            <div className="hidden dark:block absolute top-0 inset-x-0 h-px pointer-events-none bg-gradient-to-r from-transparent via-[#B89968]/45 to-transparent"></div>
          </div>
        ))}
      </div>
      ...
      <section className="flex flex-col items-center text-center max-w-2xl mx-auto mb-16 sm:mb-20 md:mb-32 space-y-5 sm:space-y-6 md:space-y-8 animate-fadeInUp px-2 sm:px-0">
        <h3 className="text-2xl sm:text-3xl md:text-4xl font-light text-[#1a1a1a] tracking-tight"
