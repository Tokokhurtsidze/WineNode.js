...
const SocialLink = ({ icon, href, ariaLabel }: { icon: ReactNode; href: string; ariaLabel: string }) => (
  <a href={href} target="_blank" rel="noopener noreferrer" aria-label={ariaLabel}
    className="w-9 h-9 border border-white/10 dark:!border-[#B89968]/45 rounded-full flex items-center justify-center text-gray-400 dark:!text-[#D9D2C6] hover:bg-[#5b1f1f] hover:border-[#5b1f1f] hover:text-white transition-all duration-500 shadow-sm">
    {icon}
  </a>
);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 mb-20">
          <div className="space-y-6">
            <Link to="/" className="text-3xl font-bold tracking-tighter italic dark:text-[#D9D2C6]" aria-label="LAMIANI Home">
              LAMIANI<span className="text-[#5b1f1f] dark:!text-[#8B0000]">.</span>
            </Link>
            <p className="text-gray-400 dark:!text-[#D9D2C6] text-sm leading-relaxed font-sans tracking-wide max-w-xs">{t.desc}</p>
          ...
          <div className="space-y-6">
            ...
            <address className="not-italic font-sans text-sm text-gray-400 dark:!text-[#D9D2C6] leading-relaxed">{t.address}</address>
            <div className="flex items-center gap-2 mt-2">
              <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
              <p className="font-sans text-[10px] text-gray-500 dark:!text-[#D9D2C6]/80 italic uppercase tracking-wider">{t.hours}</p>
            </div>
          </div>
        </div>
        <div className="h-px w-full bg-white/5 dark:bg-gradient-to-r dark:from-transparent dark:via-[#B89968]/35 dark:to-transparent mb-10" aria-hidden="true" />
